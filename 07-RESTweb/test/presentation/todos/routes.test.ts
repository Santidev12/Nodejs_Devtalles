...
describe('Todo route testing', () => {
    ...
    test('should return an updated TODO api/todos/:id', async() => {
        ...
        const { body } = await request(testServer.app)
            ...
            .send({ text: 'Hola mundo UPDATE', completedAt: '2022-12-12' })
            .expect(200)
            ...
            expect(body).toEqual(
                {
                    ...
                    completedAt: '2022-12-12T00:00:00.000Z'
                  }
            )
    });
    ...
    test('should return an updated TODO only completedAt api/todos/:id', async() => {
    ...
    test('should returnan updated TODO only text api/todos/:id', async() => {
        
        const todo = await prisma.todo.create({ data: todo1 })

        const { body } = await request(testServer.app)
            .put(`/api/todos/${ todo.id }`)
            .send({ text: 'Hola mundo UPDATE NEW' })
            .expect(200)

            expect(body).toEqual(
                {
                    id: expect.any(Number),
                    text: 'Hola mundo UPDATE NEW',
                    completedAt: todo.completedAt
                  }
            )
    });
